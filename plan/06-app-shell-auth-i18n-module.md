...
- **Russisch migriert.** Die russische Übersetzung kam mit dem Weblate-Merge (`LocalizationStrings.ru.resx`, 254 von 254) und ist als `ru.json` übernommen. Die Module bekommen `ru.json` mit leeren Werten. `GET /api/platform/locales` liefert dafür jetzt zusätzlich `ru-RU` („Русский (Russian)“); der Blazor-`LocalizationCultureMapper` führt `ru-RU` wie die anderen Sprachen mit vollem Code.
